 ...
 * 電子署名関連のツールクラス。<br>
 * 
 * KMSを使って電子署名のための暗号鍵が作成されており、それを使う場合のツール。<br>
 * 公開鍵をもとにCSR（証明書署名要求）およびテスト用に自己署名証明書を生成する。<br>
 */
@Slf4j
...
class DigitalSignCertificateToolByExistingKeyTest {
    ...
    void createKeyAndCertificate() throws IOException {
        ...
        // 既存のエイリアスを指定してキーIDを取得
        final KeyInfo keyInfo = keyManager.findKeyByAlias(keyAlias);
        appLogger.debug("Key ID: {}", keyInfo.getKeyId());
