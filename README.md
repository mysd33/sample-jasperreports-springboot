...
* 今後、帳票を追加していく予定です
    ...
    * 請求書
        ...
        * [PDF電子署名](#pdfへの電子署名付与)の設定例として、PDFに電子署名を付与した版も用意しています。（ただし、公開鍵に対する証明書が自己署名証明書を使っているため、Acrobat Readerで開くと署名の検証で完全性が不明のため、「少なくとも１つの署名に問題があります。」の表示が出ます。）            
            * [署名付与PDFファイル（基本署名での可視署名付き-ECDSA　※PKCS#12ファイルで署名）](pdf/請求書_基本_可視署名_ECDSA.pdf)
            * [署名付与PDFファイル（基本署名での可視署名付き-RSA　※PKCS#12ファイルで署名）](pdf/請求書_基本_可視署名_RSA.pdf)
            * [署名付与PDFファイル（PAdES署名での可視署名付き-ECDSA　※PKCS#12ファイルで署名）](pdf/請求書_PAdES_可視署名_ECDSA.pdf)
            * [署名付与PDFファイル（PAdES署名での可視署名付き-RSA　※PKCS#12ファイルで署名）](pdf/請求書_PAdES_可視署名_RSA.pdf)
            * [署名付与PDFファイル（PAdES署名での可視署名付き-ECDSA　※KMSで署名）](pdf/請求書_PAdES_可視署名_ECDSA_KMS.pdf)
            * [署名付与PDFファイル（PAdES署名での可視署名付き-RSA(RSASA-PSS)　※KMSで署名）](pdf/請求書_PAdES_可視署名_RSA_KMS.pdf)
...
## サブレポートの扱い
> [!WARNING]
> 現状、サブレポートを使ったサンプル様式、サンプルコードは用意できていません。 
> 今後、作成予定です。
...
* また、`DataSource Expression`で、JRBeanCollectionDataSource等のデータソースを指定して、メインの帳票様式に受け渡したデータをトップとしたプロパティパスをフィールドデータ（`$F`）を指定します。
    ...
    * フレームワークを使ったサンプルAPでのパラメータの渡し方の例
...
public class InvoiceReportCreatorWithSign extends AbstractJasperReportCreator<Order> implements InvoiceReportCreator {
    ...
    public ReportFile createInvoice(Order order) {
        ...
        Report signedReport = reportSigner.sign(report, SignOptions.builder()//
                ...
                //.visibleSignText("署名者")// 可視署名のテキスト(OpenPDFは未サポート)
